...
<NameAbbreviator class="bg-blue-800 px-2 rounded-full scale-75 text-white"
        :full-name="findSchedule(classroom.id, period)?.cst?.teacher?.name"
        separator=" "
        :letters_count="2"
    />
                                       <!-- {{ findSchedule(classroom.id, period)?.cst?.teacher?.name }} -->
                                       {{ findSchedule(classroom.id, period)?.cst?.subject?.name }}
</div>
...
const handleSubmit = async ({ form, onSuccess, onError }) => {
    ...
    // Get the first record to extract school_id and copy_id
    const firstRecord = props.records?.[0];
    console.log('firstRecord', firstRecord);
    // Create a new form object with the required IDs
    const formData = {
        ...form,  // spread the existing form data
        school_id: firstRecord?.school_id,  // safely access school_id
        copy_id: firstRecord?.copy_id , // get copy_id from the same record
        cst: firstRecord?.cst ,
        firstRecord: firstRecord // get copy_id from the same record
    };
    ...
    try {
        ...
        console.error('Submission error:', error.response?.data); // Debug log
