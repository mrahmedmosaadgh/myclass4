                    ...
                    <tr v-for="classroom in groupedSchedules" :key="classroom.id">
                        ...
                        <template v-for="day in days" :key="`${classroom.id}-${day}`">
                            <td v-for="period in periods"
                                ...
                                <div class="p-0"
                                 :style="`background-color: ${classroom.color_bg};color:${classroom.color_text}`"
                                >
                                <details v-if="get_correct_session(classroom.id,day,period)">
                                    {{ get_correct_session(classroom.id,day,period)}}
...
const get_correct_session = (classroomId, day, period) => {
    if (!props.records2) {
        return null;
    }
    ...
    const schedule = props.records2.find(schedule =>
    ...
    // Return the schedule information you want to display
    return {
        subject: schedule.cst?.subject?.name,
        teacher: schedule.cst?.teacher?.name,
        color_bg: schedule.cst?.subject?.color_bg,
